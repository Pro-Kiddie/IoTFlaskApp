...
devices = [device.device_id for device in Device.scan()] # everytime there is new device added (not so freq) need to restart bot to retrieve all devices

telegram_bot = telepot.Bot(config["telegram_bot_token"])

# MQTT client to publish messages
mqtt_client = AWSIoTMQTTClient("WebClient-1726992")
        ...
        try:
            ...
            if (result != None) and (None not in result.groups()):
                ...
                # Turn on/off buzzer for the device id specified
                    ...
                    if device_id in devices:
                        ...
                        telegram_bot.sendMessage(telegram_chat_id, "Invalid Device ID: {}".format(device_id))
                return
            ...
            if (result != None) and (None not in result.groups()):
                ...
                if device_id == "all":
                    for device in devices:
                        ...
                        telegram_bot.sendMessage(telegram_chat_id, reply)
            ...
            if (result != None) and (None not in result.groups()):
                ...
                if device_id == "all":
                    ...
                    if device_id in devices:
                        mqtt_client.publish("status/{}/takephoto".format(device_id), json.dumps({"comp" : "camera", "status" : "take"}), 1)
...
MessageLoop(telegram_bot, _telegram_respond).run_as_thread()
print("***********************************************************")
